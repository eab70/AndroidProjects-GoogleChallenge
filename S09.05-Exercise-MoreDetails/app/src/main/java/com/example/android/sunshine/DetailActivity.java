...
//  (18) Create a String array containing the names of the desired data columns from our ContentProvider
    public static final String[] weatherInfo =  {
...
//  (19) Create constant int values representing each column name's position above
    ...
    public static final int INDEX_WEATHER_DIRECTION = 7;


//  (20) Create a constant int to identify our loader used in DetailActivity
    private static final int ID_DETAIL_LOADER = 365;
...
//      (13) Find each of the TextViews by ID
        ...
        mHigh = (TextView) findViewById(R.id.tv_high_temp);
        mLow = (TextView) findViewById(R.id.tv_low_temp);
...
//      (35) Initialize the loader for DetailActivity
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        String highTempText = SunshineWeatherUtils.formatTemperature(this,highTemp);
        mHigh.setText(highTempText);

        // (29) Display the low temperature
        double lowTemp = data.getDouble(INDEX_WEATHER_MIN_TEMP);
        String lowTempText = SunshineWeatherUtils.formatTemperature(this,lowTemp);
        mLow.setText(lowTempText);
        ...
        float speed = data.getInt(INDEX_WEATHER_WIND);
        //Hadn't realized needed wind direction and that there was a util for formatting wind
        float windDirection = data.getFloat(INDEX_WEATHER_DIRECTION);
        String windText = SunshineWeatherUtils.getFormattedWind(this, speed, windDirection);
        mWind.setText(windText);

        // (32) Display the pressure
        float pressure = data.getInt(INDEX_WEATHER_PRESSURE);
        String pressureText = getString(R.string.format_pressure, pressure);
        mPressure.setText(pressureText);
