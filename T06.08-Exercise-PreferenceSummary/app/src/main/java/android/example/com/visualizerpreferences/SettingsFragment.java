...
public class SettingsFragment extends PreferenceFragmentCompat implements OnSharedPreferenceChangeListener {
    ...
    private void setPreferenceSummary(Preference preference , String value) {
        if (preference instanceof ListPreference) {
            ...
            //Line below is getting the index of the preference selected by the user
            int prefIndex = listPreference.findIndexOfValue(value);
            if (prefIndex >= 0) {
                //Line below is getting the labels and indexing it to the array using the value
                // in the line 2 up from here; so it's getting the label associated with that value
    ...
    // Doing this updates the change to the summary immediately in the Shape Color preference box
    // in Settings
    @Override
