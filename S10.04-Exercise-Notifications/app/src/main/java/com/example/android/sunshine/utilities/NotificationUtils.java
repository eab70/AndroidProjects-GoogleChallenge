...
//  (1) Create a constant int value to identify the notification
    private static final int WEATHER_NOTIFICATION_ID = 365;
...
//          (3) Create an Intent with the proper URI to start the DetailActivity
            Intent weatherDetailIntent = new Intent(context, DetailActivity.class);

//          (4) Use TaskStackBuilder to create the proper PendingIntent. Needed solution code for first 2 lines
            TaskStackBuilder taskStackBuilder = TaskStackBuilder.create(context);
            taskStackBuilder.addNextIntentWithParentStack(weatherDetailIntent);
            PendingIntent resultPendingIntent = taskStackBuilder
                    .getPendingIntent(0, PendingIntent.FLAG_UPDATE_CURRENT);

//          (5) Set the content Intent of the NotificationBuilder
            notificationBuilder.setContentIntent(resultPendingIntent);

//          (6) Get a reference to the NotificationManager. Didn't remember syntax for context...
            NotificationManager notificationManager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);

//          (7) Notify the user with the ID WEATHER_NOTIFICATION_ID
            notificationManager.notify(WEATHER_NOTIFICATION_ID, notificationBuilder.build());

//          (8) Save the time at which the notification occurred using SunshinePreferences
