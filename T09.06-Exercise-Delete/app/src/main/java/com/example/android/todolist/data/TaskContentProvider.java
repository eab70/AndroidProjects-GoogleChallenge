    ...
    public int delete(@NonNull Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case TASK_WITH_ID:
                ...
                //Didn't get the following line on my own; had written it similar to query intead of insert
                deletedItems = db.delete(TABLE_NAME, "_id=?", new String[]{id});
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }
        //(3) Notify the resolver of a change and return the number of items deleted
        //I didn't think to put my code in an if statement to check if row was not deleted
        if (deletedItems != 0) {
            getContext().getContentResolver().notifyChange(uri, null);
        }
        return deletedItems;
