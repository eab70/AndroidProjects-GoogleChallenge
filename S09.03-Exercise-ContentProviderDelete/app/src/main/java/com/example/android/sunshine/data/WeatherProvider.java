...
//  (1) Implement the delete method of the ContentProvider
    /**
...
//      (2) Only implement the functionality, given the proper URI, to delete ALL rows in the weather table
//      (3) Return the number of rows deleted

        // Get access to the database and write URI matching code
        final SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        ...
        switch (match) {
            case CODE_WEATHER:
                deletedItems = db.delete(TABLE_NAME, "1", null);
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }
