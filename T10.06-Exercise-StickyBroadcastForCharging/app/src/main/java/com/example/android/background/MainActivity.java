...
public class MainActivity extends AppCompatActivity implements
    ...
    protected void onResume() {
        ...
        // (1) Check if you are on Android M or later, if so...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            // (2) Get a BatteryManager instance using getSystemService()
            // (3) Call isCharging on the battery manager and pass the result on to your show
            // charging method
            BatteryManager batteryManager = (BatteryManager) getSystemService(BATTERY_SERVICE);
            boolean isCharging = batteryManager.isCharging();
            showCharging(isCharging);
        } else {
            // (4) If your user is not on M+, then...
            // (5) Create a new intent filter with the action ACTION_BATTERY_CHANGED. This is a
            // sticky broadcast that contains a lot of information about the battery state.
            // (6) Set a new Intent object equal to what is returned by registerReceiver, passing in null
            ...
            //(7) Get the integer extra BatteryManager.EXTRA_STATUS. Check if it matches
            ...
            boolean isCharging = batteryStatus == BATTERY_STATUS_CHARGING || batteryStatus == BATTERY_STATUS_FULL;

            //(8) Update the UI using your showCharging method
            showCharging(isCharging);
