...
//  (5) Create static constant integer values named CODE_WEATHER & CODE_WEATHER_WITH_DATE to identify the URIs this ContentProvider can handle
    public static final int CODE_WEATHER = 100;
    public static final int CODE_WEATHER_WITH_DATE = 101;

//  (7) Instantiate a static UriMatcher using the buildUriMatcher method
    private static final UriMatcher sUriMatcher = buildUriMatcher();

    WeatherDbHelper mOpenHelper;

    // (6) Write a method called buildUriMatcher where you match URI's to their numeric ID
    public static UriMatcher buildUriMatcher() {
        final UriMatcher matcher = new UriMatcher(UriMatcher.NO_MATCH);

        //These are the URI matches, that is, the URIs to be recognized and the integer
        //constants they'll match with.
...
//  (1) Implement onCreate
    @Override
    public boolean onCreate() {
        Context context = getContext();

//      (2) Within onCreate, instantiate our mOpenHelper
        mOpenHelper = new WeatherDbHelper(context);

//      (3) Return true from onCreate to signify success performing setup
        return true;
    }
...
// (8) Provide an implementation for the query method
    /**
...
//      (9) Handle queries on both the weather and weather with date URI//
        switch (match) {
            case CODE_WEATHER:
                retCursor = db.query(TABLE_NAME,         // The table to query
            ...
            case CODE_WEATHER_WITH_DATE:
                //The following code required help from the solution code, because I didn't know
                // what the URI being sent to the provider would look like (according to the solution notes it
                // would look something like this - content://com.example.android.sunshine/weather/1472214172 -
                //where the last part of the path is a UTM timestamp).
                String mSelection = COLUMN_DATE + " = ? ";
                String normalizedUtcDateString = uri.getLastPathSegment();
                String[] mSelectionArgs = new String[]{normalizedUtcDateString};

                retCursor = db.query(TABLE_NAME,  // WeatherContract.WeatherEntry.TABLE_NAME,
                        projection,
                        mSelection,
                        mSelectionArgs,
                        null,
        ...
        // (10) Call setNotificationUri on the cursor and then return the cursor
